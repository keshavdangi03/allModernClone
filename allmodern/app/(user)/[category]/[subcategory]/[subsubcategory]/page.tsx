...
async function resolvePageData(categorySlug: string, subcategorySlug: string, subsubcategorySlug: string) {
  ...
  // Find column section matching subcategorySlug
  ...
  // Find sub-link matching subsubcategorySlug inside that section
  const matchedLink = section.links.find(
    (l: string) => slugifyNavLink(l) === subsubcategorySlug
  );
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const data = await resolvePageData(category, subcategory, subsubcategory);
  if (!data) return { title: "Page Not Found | AllModern" };
  return {
    title: `${data.title} | ${data.parentTitle} | AllModern`,
    description: data.description,
  };
}

export default async function SubSubCategoryPage({ params }: PageProps) {
  const { category: categorySlug, subcategory: subcategorySlug, subsubcategory: subsubcategorySlug } = await params;
  ...
  if (collectionsRedirects[subsubcategorySlug]) {
    redirect(collectionsRedirects[subsubcategorySlug]);
  }

  const data = await resolvePageData(categorySlug, subcategorySlug, subsubcategorySlug);
    ...
    <main className="bg-white min-h-screen pb-16">
      ...
      <div className="bg-slate-50 border-b border-slate-200 py-3">
        <div className="mx-auto max-w-[1400px] px-4 sm:px-6 text-[12px] text-slate-500 flex items-center gap-1.5 font-medium">
          ...
          <span>/</span>
          <Link href={`/${categorySlug}/${subcategorySlug}`} className="hover:text-slate-900 transition">{data.parentTitle}</Link>
          <span>/</span>
